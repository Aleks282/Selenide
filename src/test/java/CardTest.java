...
public class CardTest {
    ...
    void shouldSendFormWithInvalidPhoneNumber() {
        ...
        $("[data-test-id=agreement]").click();
        $(".button").click();
        $("[data-test-id=phone] .input__sub").shouldHave(exactText("Телефон указан неверно. Должно быть 11 цифр, например, +7900000000."));
    }
    ...
    void shouldSendFormWithInvalidDate() {
        $("[data-test-id=city] input").setValue("Москва");
        $("[data-test-id=date] input").doubleClick().sendKeys("15.03.2023");
        $("[data-test-id=name] input").setValue("Иванов Иван");
        $("[data-test-id=phone] input").setValue("+79251111111");
        $("[data-test-id=agreement]").click();
        $(".button").click();
